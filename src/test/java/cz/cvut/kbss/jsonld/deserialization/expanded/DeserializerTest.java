...
class DeserializerTest {
    ...
    void resolveTargetClassReturnsExpectedClassWhenItIsPlainIdentifierType() throws Exception {
        final Deserializer<JsonObject> deserializer = new ObjectDeserializer(null,
                                                                             new DeserializerConfig(new Configuration(),
                                                                                                    new TargetClassResolver(
                                                                                                            new TypeMap()),
                                                                                                    new CommonValueDeserializers()),
                                                                             URI.class);
        assertEquals(URI.class,
                     deserializer.resolveTargetClass(TestUtil.readAndExpand("objectWithDataProperties.json").getJsonObject(0),
                                                     URI.class));
    }
}